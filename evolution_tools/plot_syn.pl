#!/usr/bin/perl
# 2016-06-23 It seems that the softwares which convert SVG to PNG cannot deal with 'alignment-baseline' correctly, so I want to ignore using it. 
use strict; 
use warnings; 
use SVG; 
use LogInforSunhh; 
...
$default_string{'frame_setopt'} = [
	...
	'xAxis_LabFontSize=20;xAxis_LabFontFam=ArialNarrow;xAxis_LabFontWeight=bold;xAxis_LabHoriAln=middle;', 
	...
	'yAxis_TickStep=10000000;yAxis_TickUnit=M;yAxis_TickLwd=1;yAxis_TickLen=3;yAxis_TickCol=blue;yAxis_TickFontSize=0;yAxis_FontWeight=lighter;yAxis_FontFam=ArialNarrow;yAxis_TxtHoriAln=middle;', 
	'yAxis_BpPoint=300000;yAxis_BorderLwd=1;yAxis_BorderCol=black;yAxis_SepBorder=0;', 
	'yAxis_LabFontSize=20;yAxis_LabFontFam=ArialNarrow;yAxis_LabFontWeight=bold;yAxis_LabHoriAln=middle;', 
...
if (@{$blk_opt{'lks_col_rgb'}} > 1 and $blk_opt{'lks_col_min'} < $blk_opt{'lks_col_max'}) {
	...
	$svg->text(
		...
		'y' => $frame_opt{'scaleBar_y0'} + $frame_opt{'scaleBar_height'} * 0.8 , 
	...
	for ( my $yi = 0; $yi < @{$chrLisY{'arr'}} ; $yi ++ ) {
		...
		if ( $yi == 0 and defined $grps{'xAxis_Tick'} ) {
			for ( my $i=0; $i<=$chrX_len; $i+=$frame_opt{'xAxis_TickStep'} ) {
				...
				my $tmp_xV = $i / $frame_opt{'xAxis_BpPoint'}; 
				my $tmp_yV = $frame_opt{'xAxis_TickLen'} ;
				$grps{'xAxis_Tick'}->text(
					'id' => "xAxis_tickTxt:$chrX_ID:$chrX_rI:$i", 
					'x'  => $tmp_xV, 'y'  => $tmp_yV + $frame_opt{'xAxis_TickFontSize'}, 
					-cdata => $show_v, 
					'transform' => join('', "translate($sP_x $sP_y)"), 
					'stroke' => 'none', 
		...
		if ( $yi == 0 and $frame_opt{'xAxis_ChrIDFontSize'} > 0 ) {
			my $tmp_xV = ($sP_x + $eP_x)/2 ; 
			my $tmp_yV = $frame_by + $frame_opt{'xAxis_TickLen'} + $frame_opt{'xAxis_TickFontSize'} + $frame_opt{'xAxis_ChrIDFontSize'} * 0.3 ; 
			$grps{'xAxis_Label'}->text(
				'x'    => $tmp_xV , 
				'y'    => $tmp_yV + $frame_opt{'xAxis_ChrIDFontSize'} , 
				-cdata => "$chrX_ID" , 
		...
		if ( $yi == 0 and @{$shad_opt{'xAxis_shadow_setopt'}} > 0 ) {
			...
			for ( my $j=0; $j<@{$shad_opt{'xAxis_shadow_setopt'}}; $j++ ) {
				...
				if ( $xi == $#{$chrLisX{'arr'}} ) {
					$svg->text(
						'x' => $eP_x + 3 + $shad_height * 0.8 , 'y' => $shad_yV + $shad_height , 
						-cdata => "$shad_ID" , 
						'fill'               => $shad_col, 
						'stroke'             => 'none' , 
		...
		if ( $xi == 0 and @{$shad_opt{'yAxis_shadow_setopt'}} > 0 ) {
			...
			for ( my $j=0; $j<@{$shad_opt{'yAxis_shadow_setopt'}}; $j++ ) {
				...
				if ( $yi == 0 ) {
					...
					$svg->text(
						'x' => $shad_xV , 'y' => $tmp_yV + $shad_height , 
						-cdata => "$shad_ID" , 
						'fill'               => $shad_col, 
						'stroke'             => 'none' , 
						'font-size'          => $shad_height * 0.8, 
...
if ( $img_opt{'xAxisLabel'} ne '' ) {
	my $tmp_xV = ($frame_lx + $frame_rx) / 2; 
	my $tmp_yV = $frame_by + $frame_opt{'xAxis_TickLen'} + $frame_opt{'xAxis_TickFontSize'} + $frame_opt{'xAxis_ChrIDFontSize'}*2 ; 
	$grps{'xAxis_Label'}->text(
		'x' => $tmp_xV , 'y' => $tmp_yV + $frame_opt{'xAxis_LabFontSize'} , 
