...
sub input_good {
	$gg{'usage_txt'} = <<'H1'; 
################################################################################
perl $0 -in_gvcf_list gvcf_list -conf_file pipe_gatk_conf -out_dir out/ -wrk_dir wrk/   # for an already single/VAR gVCF input
...
sub set_stepPara {
	...
	$gg{'jnGVCF_list'} = [ map { &fileSunhh::_abs_path($_->[0]) } &fileSunhh::load_tabFile( $opts{'in_gvcf_list'} ) ]; 
...
sub step9_gvcf2var {
	...
	if ( $scfID ne '' or !$gg{'para'}{'CallByScf'} ) {
		...
		# Here the gvcf file is single and it is a VAR vcf file instead of a GVCF file. 
		$cmd .= "grep -v ^\@ $scfID > $scfID.forBCF\n"; 
		$cmd .= "$cfg{'exe_bcftools'} view \\\n"; 
		$cmd .= "  $glist->[0] \\\n"; 
		$cmd .= "  --regions-file  $scfID.forBCF\\\n"; 
		$cmd .= "  --no-update     \\\n"; 
		$cmd .= "  --output-type   z \\\n"; 
		$cmd .= "  --output-file   ${opref}_rawV.vcf.gz \\\n"; 
		$cmd .= "1> s9.std.callRawV.${opref} 2> s9.err.callRawV.${opref}\n"; 
		$cmd .= "$cfg{'exe_bcftools'} index -f --tbi ${opref}_rawV.vcf.gz \n"; 
		##$cmd .= "$cfg{'exe_java'} -Xmx10g -Xms10g -jar $cfg{'jar_gatk'} -T SelectVariants \\\n"; 
		##$cmd .= "  -V $glist->[0] \\\n"; 
		##$cmd .= "  -R $cfg{'ref_fasta'} \\\n"; 
		##$cmd .= "  -L $scfID \\\n"; 
		##$cmd .= "  -o ${opref}_rawV.vcf.gz \\\n"; 
