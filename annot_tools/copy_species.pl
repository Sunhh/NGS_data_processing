...
if(!$prokaryotic){
    open (GENERIC, "<$configdir/species/${refSpec}/${refSpec}_parameters.cfg") or die ("Could not open $configdir/species/${refSpec}/${refSpec}_parameters.cfg.");
...
if(!$prokaryotic){
    if (stat "$configdir/species/${refSpec}/${refSpec}_weightmatrix.txt" == 0){
	die ("$configdir/species/${refSpec}/${refSpec}_weightmatrix.txt doesn't exist.");
    }
    system ("cp $configdir/species/${refSpec}/${refSpec}_weightmatrix.txt $weightfilename");
}else{
...
if(!$prokaryotic){
    if (stat "$configdir/species/${refSpec}/${refSpec}_metapars.cfg" == 0){
	die ("$configdir/species/${refSpec}/${refSpec}_metapars.cfg doesn't exist.");
    }
