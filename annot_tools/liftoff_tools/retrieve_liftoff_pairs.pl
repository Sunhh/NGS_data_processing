...
scalar(@ARGV) == 3 or die "perl $0 from_tag to_tag output/mapCDS.CCpan.to.CLpan.tbl\n";
...
my $min_localID = 0.9;
my $min_ovlR   = 0.5;
my $min_ovlLen = 100; # [3/29/2022] Not used now.

my @o1; # [ [ ori_R_ID, Q_ID/loc, R2Q_ovlLen, trans_R_ID, trans_R_chrID, trans_R_chrS, trans_R_chrE, trans_R_chrStr, identity, coverage, trans_R_size, trans_R_exNum, ori_R_size, ori_R_exNum, Q_size, Q_exNum, Q_ID_type, trans_R_CDSblks], ... ]
...
@o1 = sort {
  my $res = $b->[8] <=> $a->[8] || $b->[2] <=> $a->[2]; # High to low: identity, R2Q_ovlLen;
  if ($res == 0 and $a->[10] ne ".") {
    # trans_R_size-ori_R_size, trans_R_exNum-ori_R_exNum;
    $res =  abs($a->[10]-$a->[12]) <=> abs($b->[10]-$b->[12]) || abs($a->[11]-$a->[13]) <=> abs($b->[11]-$b->[13]);
  }
  if ($res == 0) {
    # trans_R_chrID, trans_R_chrS, trans_R_chrE
