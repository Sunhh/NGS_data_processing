...
my %tool; 
{
	$tool{makeblastdb} = '/usr/local/bin/makeblastdb'; 
	$tool{blastn} = '/usr/local/bin/blastn'; 
	$tool{mv} = '/usr/bin/mv'; 
...
&exeCmd("$tool{mv} $refGff.tmp.tab1 $refGff.tmp.tab"); 
...
&exeCmd("$tool{mv} $inGff.tmp.tab1 $inGff.tmp.tab"); 
...
&exeCmd("$tool{makeblastdb} -dbtype nucl -in $refGff.tmp.ltr.fa"); 
&exeCmd("blastn -outfmt \"6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore qlen slen sstrand\" -num_threads 20 -task blastn -dust no -db $refGff.tmp.ltr.fa -query $inGff.tmp.ltr.fa -out $inGff.tmp.ltr.fa.toTRs.bn6"); 
