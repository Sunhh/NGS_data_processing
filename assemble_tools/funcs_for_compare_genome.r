...
boundary_in_psl <- function ( psl=psl, id1="scaffold116_cov122", id2="scaffold12", id1.range=NULL, id2.range=NULL ) {
	id1 <- as.character(id1) 
	id2 <- as.character(id2) 
	psl <- psl[ psl$V10 == id1 & psl$V14 == id2 ,]
	if (!is.null(id1.range)) {
		id1.range <- as.numeric(id1.range)
		psl <- psl[ psl$V12+1 <= id1.range[2] & psl$V13 >= id1.range[1], ]
	}
	if (!is.null(id2.range)) {
		id2.range <- as.numeric(id2.range)
		psl <- psl[ psl$V16+1 <= id2.range[2] & psl$V17 >= id2.range[1], ]
	}
	id1.back <- list(min=c(-1,-1,-1,-1, '+'), max=c(-1,-1,-1,-1, '+'))
	if ( nrow(psl) > 0 ) {
		i.min <- order(psl$V12, decreasing=FALSE)[1]
		i.max <- order(psl$V12, decreasing=TRUE)[1]
		id1.back$min <- as.character( c( psl[ i.min, c(12,13, 16,17, 9) ] ) )
		id1.back$max <- as.character( c( psl[ i.max, c(12,13, 16,17, 9) ] ) ) 
	}
	return( id1.back )
}# boundary_in_psl() 
