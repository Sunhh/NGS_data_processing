...
boundary_in_psl <- function ( psl=psl, id1="scaffold116_cov122", id2="scaffold12", id1.range=NULL, id2.range=NULL ) {
	...
	if (!is.null(id1.range)) {
		...
		psl <- psl[ psl$V12+1 <= id1.range[1] & psl$V13 >= id1.range[2], ]
	}
	if (!is.null(id2.range)) {
		id2.range <- as.numeric(id2.range)
		psl <- psl[ psl$V16+1 <- id2.range[1] & psl$V17 >= id2.range[2], ]
	}
	id1.back <- list(min=c(-1,-1,-1,-1, '+'), max=c(-1,-1,-1,-1, '+'))
	if ( nrow(psl) > 0 ) {
		i.min <- order(psl$V12, decreasing=FALSE)[1]
		i.max <- order(psl$V12, decreasing=TRUE)[1]
		id1.back$min <- c( psl[ i.min, c(12,13, 16,17, 9) ] ) 
		id1.back$max <- c( psl[ i.max, c(12,13, 16,17, 9) ] ) 
	}
	return( id1.back )
}# boundary_in_psl() 
