...
Version   : $version
Last time : $last_time
perl $0 -opre out_prefix input1.fastq 
...
&timeLog( "[Rec] Reading $ptag file: $rf1\n" ); 
&timeLog( "[Rec] Parameters: -subseqS=$subseqS -subseq=$subseqP -rcDup=$rcDup -maxDupNum=$maxDupN -noout=$no_out -opre=$ptag\n" ); 

unless ($no_out) {
	if ($is_ogz) {
		open (O1,'|-', "gzip -c >${ptag}.ndupB.gz") or die;
		open (D1,'|-', "gzip -c >${ptag}.dropB.gz") or die; 
		&timeLog( "[Rec] Create output files: ${ptag}.ndupB.gz ${ptag}.dropB.gz\n" ); 
	}else{
		open (O1,'>', "${ptag}.ndupB") or die;
		open (D1,'>', "${ptag}.dropB") or die; 
		&timeLog( "[Rec] Create output files: ${ptag}.ndupB ${ptag}.dropB\n" ); 
	}
}
...
while (!eof(R1)) {
	...
	my $dupN; 
	if ( $rcDup ) {
		my $s_both_rc = reverse($s_both); 
		$s_both_rc =~ tr/ATGCatgc/TACGtacg/; 
		$have_both{$s_both_rc} ++; 
