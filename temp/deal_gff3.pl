...
sub getID_sameIntron_gff {
  ...
  my @topID_2 = sort {
    my $aLn = $g2->{'ID2lineN'}{$a};
    my $bLn = $g2->{'ID2lineN'}{$b};
    $g2->{'lineN2hash'}{$aLn}{'seqID'} cmp $g2->{'lineN2hash'}{$bLn}{'seqID'} ||
    $g2->{'lineN2hash'}{$aLn}{'start'} <=> $g2->{'lineN2hash'}{$bLn}{'start'} ||
    $g2->{'lineN2hash'}{$aLn}{'end'}   <=> $g2->{'lineN2hash'}{$bLn}{'end'}
  } keys %{ $g2->{'lineN_group'} }; # I should sort this according to their location. I don't care about the strand here to be compatible with unstranded RNA-seq data.
  my %chr2pid_2; 
  for my $topID (@topID_2) {
    my $chr = &id_to_chr($g2, $topID); 
    push(@{$chr2pid_2{$chr}}, $topID); 
  }
  my (%topID_SE_1, %topID_SE_2);
  for my $tid (@topID_1) {
    my $lN = $g1->{'ID2lineN'}{$tid};
    $topID_SE_1{$tid} = [ $g1->{'lineN2hash'}{$lN}{'start'}, $g1->{'lineN2hash'}{$lN}{'end'} ];
  }
  for my $tid (@topID_2) {
    my $lN = $g2->{'ID2lineN'}{$tid};
    $topID_SE_2{$tid} = [ $g2->{'lineN2hash'}{$lN}{'start'}, $g2->{'lineN2hash'}{$lN}{'end'} ];
  }
  # Output variables. 
  my @back_topIDs; 
  # Detect good topIDs.
  ...
  CHK_TOPID_1: for my $tid1 ( @topID_1 ) {
    ...
    my %intronLoc = &interval_locLis( $exLocLis{'locLis'}, 0 ); 
    my $exLen = &mathSunhh::_sum( map { $_->[1]-$_->[0]+1 } @{$exLocLis{'locLis'}});
    CHK_TOPID_2: for my $tid2 ( @{$chr2pid_2{$chr1}} ) {
      $topID_SE_2{$tid2}[0] > $topID_SE_1{$tid1}[1] and last;
      $topID_SE_2{$tid2}[1] < $topID_SE_1{$tid1}[0] and next;
      my %intronLoc_2; 
